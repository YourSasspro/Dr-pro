...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth()->user()->role=='doctor' || Auth()->user()->role=='dentist'){
            ...
            $total_patients = User::where('role','patient')->where('doctor_id',$doctor_id)->count();
        $total_patients_today = User::where('role','patient')->where('doctor_id',$doctor_id)->wheredate('created_at', Today())->count();
        $total_appointments = Appointment::where('doctor_id',$doctor_id)->count();
        $total_appointments_today = Appointment::wheredate('date', Today())->where('doctor_id',$doctor_id)->get();
        $total_prescriptions = Prescription::where('doctor_id',$doctor_id)->count();
        $total_payments = Billing::where('doctor_id',$doctor_id)->count();
        $total_payments = Billing::where('doctor_id',$doctor_id)->count();
